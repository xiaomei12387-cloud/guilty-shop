...
function handleProfileUpdate(e) {
  ...
  const nameInp = document.getElementById("profNameInput") || document.getElementById("profEditName");
  const agentIdInp = document.getElementById("profAgentIdInput") || document.getElementById("profEditAgentId");
  const locInp = document.getElementById("profLocationInput");

  const newName = nameInp ? nameInp.value.trim() : (memberProfile.name || "特工");
  const newAgentId = agentIdInp ? agentIdInp.value.trim().toUpperCase() : (memberProfile.agentId || "AGENT-001");
  const newLoc = locInp ? locInp.value.trim() : (memberProfile.defaultLocation || "");
  ...
  toggleAuthModal(false);
  if (typeof closeEditProfileDrawer === "function") closeEditProfileDrawer();
  alert("✔ 特工檔案數據已成功更新！");
}
